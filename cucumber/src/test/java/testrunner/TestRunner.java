...
public class TestRunner {
  ...
  public static void runThisBeforeTestExecution() throws Exception {
    ...
    testEnv =
      System.getProperty("env") == null ? DEFAULT_ENV : System.getProperty("env").toLowerCase();
    ...
    for (File file : files) {
      if (
        file.getAbsolutePath().endsWith(testEnv + File.separator + "config.properties") ||
        file.getAbsolutePath().endsWith("common" + File.separator + "config.properties")
      ) {
        try {
